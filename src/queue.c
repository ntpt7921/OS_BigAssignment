...
static inline bool
compare_pcb_smaller (pcb_t *p_elem1, pcb_t *p_elem2)
{
...
static inline void
add_to_end (queue_t *queue, pcb_t *proc)
{
...
static inline void
swap_start_with_end (queue_t *queue)
{
  swap_elem (&queue->proc[0], &queue->proc[queue->size - 1]);
}

static inline int
get_parent_index (int child_index)
{
  if (child_index == 0)
    return 0;
  else
...
static inline int
get_left_child_index (int parent_index)
{
  return (2 * parent_index + 1);
}

static inline int
get_right_child_index (int parent_index)
{
  return (2 * parent_index + 2);
}

static void
sift_up (queue_t *queue, int index)
{
...
static void
sift_down (queue_t *queue, int index)
{
  int current_index = index;
