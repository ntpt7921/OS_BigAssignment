...
static struct
{
  ...
  int index;     // Index of the page in the list of pages allocated to the
                 // process.
...
void
init_segment_table (struct seg_table_t *s_table)
{
  s_table->size = 0;
...
void
init_page_table (struct page_table_t *p_table)
{
  p_table->size = 0;
...
static void
set_page_table_entry (struct pcb_t *proc, uint32_t page_num,
                      uint32_t virt_addr)
{
...
static void
free_page_table_entry (addr_t addr, struct pcb_t *proc)
{
...
alloc_mem (uint32_t size, struct pcb_t *proc)
{
  ...
  uint32_t num_pages = size / PAGE_SIZE + ((size % PAGE_LEN) ? 1 : 0);
  ...
  if (mem_avail)
    {
      ...
      for (uint32_t i = 0; alloc_frame_count < num_pages && i < NUM_PAGES; i++)
        {
